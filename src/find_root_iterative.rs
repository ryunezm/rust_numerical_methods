...
{ function: fn(f64) -> f64, x0: f64, x1: f64, x2: f64, tolerance: f64, max_iter: usize }
...
impl FixedPoint {
    fn new(function: fn(f64) -> f64, x0: f64, tolerance: f64, max_iter: usize)
        -> Self { FixedPoint{ function, x0, tolerance, max_iter } }
}
    ...
    fn solve(&mut self) -> f64 {
        ...
        while iter < self.max_iter {
           let x_next = (self.function)(x0);
            ...
            if (x_next - x0).abs() < self.tolerance { return x_next; }
...
impl InverseQuadraticInterpolation{
    fn new(function: fn(f64) -> f64, x0: f64, x1: f64, x2: f64, tolerance: f64, max_iter: usize)
        ->Self{ InverseQuadraticInterpolation{ function, x0, x1, x2, tolerance, max_iter } }
}
...
impl FindRoot for InverseQuadraticInterpolation {
    fn solve(&mut self) -> f64 {
        ...
        while iter < self.max_iter {
            ...
            if (x_next - x1).abs() < self.tolerance { return x_next; }
